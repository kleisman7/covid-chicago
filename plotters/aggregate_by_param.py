...
def plot_sim(exp_name,exp_path,grp_list, param, channel):
    ...
    first_day = pd.Timestamp('2020-03-25')
    last_day = pd.Timestamp.today()
    
    column_list =['date', 'covid_region']
    ...
    for c, grp in enumerate(list(df['covid_region'].unique())):
        mdf = df[df['covid_region']==grp]
...
def aggregate_trajectories(exp_name, grp, param=None, channels=None,verbose=False):
    ...
    column_list.append('N_' + str(grp.replace("-", "_")))
    df = load_sim_data(exp_name,region_suffix = region_suffix,column_list=column_list,  add_incidence=True,calc_prevalence=True,verbose=verbose)
    df['covid_region'] = grp
    if param is not None:
    ...
    groupby_channels = ['date', 'startdate', 'time', 'covid_region',param,'N']
    groupby_channels = list(set(groupby_channels))
    channels = [ch for ch in list(df.columns) if ch not in
                list(set(groupby_channels + ['scen_num','sample_num','run_num','N']))]
...
if __name__ == '__main__':
    ...
    if not plot_only:
        ...
        outcome_channels = ['infected','deaths','infected_cumul','deaths_det_cumul']
        if len(grp_list) >1:
            dfAll = pd.DataFrame()
            for grp in grp_list:
                print(f'Start aggregating trajectories for {grp}')
                df = aggregate_trajectories(exp_name,grp, param=param, channels=outcome_channels,verbose=True)
            ...
            df = aggregate_trajectories(exp_name,grp=grp_list[0], param=param,channels=outcome_channels,verbose=True)
            df['scenario_name'] = exp_name.split("_")[-1]
            df['simdate'] = exp_name.split("_")[0]
    ...
    #plot_sim(exp_name,exp_path,grp_list, param, channel='prevalence')
